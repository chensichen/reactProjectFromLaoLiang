...
export default class productList extends React.Component {
  ...
  render() {
    ...
    const row = (rowData, sectionID, rowID) => {
      ...
      return (
        <div key={rowID} style={{padding: '0 1em 0 0'}}>
          <div style={{display: 'flex', padding: '1em 0', justifyContent: 'space-between'}}>
            <span className='rowNum'>{rowID - 1 + 2}</span>
            <img style={{height: '6em', width: '6em', marginRight: '0.5em'}} src={obj.url_show} alt="" onClick={() => {
              this.myProductDetail(obj.id)
            }}/>
            <div style={{width: '48%'}}>
              <div onClick={() => {this.myProductDetail(obj.id)}}>
                <div className='productTitle' dangerouslySetInnerHTML={{__html: obj.name}}/>
              </div>
              <div
                className='productTap'
                onClick={() => {this.myProductDetail(obj.id)}}>
                {obj.promotion}
              </div>
              <div><span className='productMoney' onClick={() => {this.myProductDetail(obj.id)}}>￥{obj.price}</span></div>
              <div
                className='productDetailNum'
                onClick={() => {
                this.myProductDetail(obj.id)
              }}>已售{obj.base_volume}{obj.unit}</div>
            </div>
            <img
    ...
    return (
      <div className='productList' style={{background:'#ffffff'}}>
        <ListView
          dataSource={this.state.dataSource}
          renderFooter={() => (<div style={{textAlign: 'center', fontSize:'0.8em',}}>
